...
class Palette {
    ...
    makeGrayscale() {
        for(let i = 0; i < this.size; i++) {
            let c = this.colors[i];
            let l = (red(c) + green(c) + blue(c))/3;
            this.colors[i] = color(l);
        }
    }
    ...
    desaturate(amount) {
        ...
        for(let i = 0; i < this.size; i++) {
            ...
            let newS = lerp(s, 0, amount);
            this.colors[i] = color(h, newS, b);
...
function allPalettes() {
    return [...basicBank(), ...porcelainBank(), ...albersBank(), ...bauhausBank()];
}
